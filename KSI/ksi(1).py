...
def mouse(event, x, y, flags, param):
    ...
    if event == cv.EVENT_LBUTTONDOWN:
        onx = x - 488 # 오른쪽 영상의 x좌표이므로 왼쪽 영상의 넓이 488을 빼준다.
        ony = y
...
if cap1.isOpened() and cap2.isOpened():
    ...
    while ret1 or ret2:
        ...
        if heflag == 1:
            ...
            #메디안 필터링
            #md_image = cv.medianBlur(src = frame2, ksize=5)

            #언샤프 마스킹
            # 블러 영상
            blur = cv.GaussianBlur(src=frame2, ksize=(21, 21),
                                   sigmaX=3, borderType=cv.BORDER_REPLICATE)
            # 원본에서 블러 빼기
            UnsharpMaskImg = frame2 - blur
            # 이를 원본에 더하여 언샤프마스킹 마무리
            frame2 = frame2 + UnsharpMaskImg
