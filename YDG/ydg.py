...
if cap1.isOpened() and cap2.isOpened():
    ...
    while ret1 or ret2:
        ...
        if mousesq.getminy()-mousesq.getmaxy()!=0 and mousesq.getminx()-mousesq.getmaxx()!=0:
            ...
            # 히스토그램 평활화
            if heflag==1:
                yuv_image = cv.cvtColor(frame2[mousesq.getminy():mousesq.getmaxy(), mousesq.getminx():mousesq.getmaxx()], cv.COLOR_BGR2YUV)
            ...
            # 가우시안 블러링
            if sigma!=0:
                blur_image = cv.GaussianBlur(frame2[mousesq.getminy():mousesq.getmaxy(), mousesq.getminx():mousesq.getmaxx()], (0,0),sigma)
                frame2[mousesq.getminy():mousesq.getmaxy(), mousesq.getminx():mousesq.getmaxx()] = blur_image

            # 언샤프 마스킹
            if un_sigma!=0:
                blur = cv.GaussianBlur(frame2[mousesq.getminy():mousesq.getmaxy(), mousesq.getminx():mousesq.getmaxx()], ksize=(21, 21),sigmaX=un_sigma, borderType=cv.BORDER_REPLICATE)
        ...
        text_frame2 = cv.putText(frame2, "this_index="+str(count), text_pos, font, font_scale,font_color, font_thickness)
        count+=1
